...
public class FindAverageSalary {
    public static void main(String[] args) {
        ...
        Map<String, Double> averageSalaryByDepWihtStream = employees.stream()
                .collect(Collectors.groupingBy(Employee::getDepartment,
                        Collectors.averagingDouble(Employee :: getSalary)));

        averageSalaryByDepWihtStream.forEach((dep,averSalary)->
                System.out.println(dep + "->  $" + averSalary ));

        System.out.println("______________________________________________");


        for (Map.Entry<String,Double> entry : averageSalaryByDepWihtStream.entrySet()){
